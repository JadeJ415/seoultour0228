...
@st.cache_data
def load_csv(file_name, encoding='utf-8-sig'):
    ...
    try:
        ...
        st.error(f"데이터 파일이 비어있습니다: {file_name}. GitHub에 파일이 제대로 업로드되었는지 확인해주세요.")
        return pd.DataFrame()
    except Exception as e:
        try:
            return pd.read_csv(path, encoding='cp949')
        except pd.errors.EmptyDataError:
            st.error(f"데이터 파일이 비어있습니다: {file_name}")
            return pd.DataFrame()
        except Exception as e2:
            st.error(f"데이터 로드 오류 ({file_name}): {e2}")
            return pd.DataFrame()

@st.cache_data
def load_data():
    # 1. 국적 데이터
    df_nat_raw = load_csv('(전국기준)국적별+외국인+방문객_20260228095700.csv')
    if not df_nat_raw.empty:
        ...
        df_nat = pd.DataFrame(columns=['대륙', '국가', '계', '남자', '여자'])
    
    # 2. 연령 데이터
    df_age_raw = load_csv('(전국기준)연령별+외국인+방문객_20260228095859.csv')
    ...
    # 3. 교통 데이터
    df_trans_raw = load_csv('(전국기준)입국교통수단별+외국인+방문객_20260228095722.csv')
    ...
    # 4. 호텔 데이터
    df_hotel_raw = load_csv('관광호텔+등록현황_20260228095634.csv')
    if not df_hotel_raw.empty:
        ...
        df_hotel = pd.DataFrame(columns=['지역1', '지역2', '호텔수', '객실수'])
    ...
    if os.path.exists(db_path):
        try:
            ...
            df_airbnb = pd.DataFrame(columns=['price_value', 'price_val'])
    else:
        st.error("airbnb.db 파일을 찾을 수 없습니다.")
        df_airbnb = pd.DataFrame(columns=['price_value', 'price_val'])

    # 6. 도시민박 데이터
    df_fore = load_csv('foreigner.csv')
